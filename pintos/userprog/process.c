...
tid_t process_create_initd(const char* file_name) {
  ...
  if (tid == TID_ERROR) palloc_free_page(file_name_copy);
  if (cs != NULL) {
    list_remove(&cs->elem);
    free(cs);
  }
...
static void initd(void* f_name) {
  ...
  if (curr == NULL || args == NULL) thread_exit();

#ifdef VM
  supplemental_page_table_init(&curr->spt);
#endif

  if (thread_root() == NULL) thread_set_root(curr);
  ...
  if (process_exec(f_name) < 0) thread_exit();
  NOT_REACHED();
}
/*
...
static bool duplicate_fd_table(struct thread* curr, struct thread* parent) {
  ...
  if (curr == NULL || parent == NULL) return false;

  curr->fd_table = palloc_get_page(PAL_ZERO);
  if (curr->fd_table == NULL) return false;
  ...
  for (fd = 2; fd < parent->next_fd && fd < FD_MAX; fd++) {
    if (parent->fd_table[fd] == NULL) continue;
...
error:
  ...
  for (fd = 2; fd < FD_MAX; fd++) {
    if (curr->fd_table[fd] == NULL) continue;
...
static bool duplicate_running_file(struct thread* curr, struct thread* parent) {
  ...
  if (parent->running_file == NULL) return true;
...
static void __do_fork(void* aux) {
  ...
  if (!duplicate_fd_table(curr, parent)) goto error;

  if (!duplicate_running_file(curr, parent)) goto error;
...
int process_wait(tid_t child_tid) {
  ...
  for (e = list_begin(childs); e != list_end(childs); e = list_next(e)) {
    ...
    if (cs->tid != child_tid) continue;

    if (cs->waited) return -1;

    cs->waited = true;

    if (!cs->exited) sema_down(&cs->wait_sema);
...
static void close_open_files(struct thread* curr) {
  ...
  for (fd = 2; fd < FD_MAX; fd++) process_close_file(fd);
...
static void finish_self_status(struct thread* curr) {
  ...
  if (curr == NULL || curr->self_status == NULL) return;
...
void process_exit_with_status(int status) {
  ...
  if (curr->self_status != NULL) curr->self_status->exit_status = status;
 ...
 */
static void close_running_file(struct thread* curr) {
  if (curr == NULL || curr->running_file == NULL) return;
...
void process_exit(void) {
  ...
  if (thread_root() == curr) thread_set_root(NULL);
#ifndef VM
  process_cleanup();
#endif
}
...
int process_add_file(struct file* f) {
  ...
  if (curr == NULL || f == NULL || curr->fd_table == NULL) return -1;
...
static void process_close_file_unlocked(int fd) {
  ...
  if (curr == NULL || fd < 2 || fd >= FD_MAX || curr->fd_table == NULL) return;

  // fd 테이블에 파일이 없으면
  if (curr->fd_table[fd] == NULL) return;
  file_close(curr->fd_table[fd]);
  curr->fd_table[fd] = NULL;
  if (fd < curr->next_fd) curr->next_fd = fd;
}
...
static bool validate_segment(const struct Phdr* phdr, struct file* file) {
  ...
  lock_release(&filesys_lock);
  if (phdr->p_offset > (uint64_t)length) return false;
...
static bool load(const char* file_name, struct intr_frame* if_) {
  ...
  while (token != NULL) {
    if ((size_t)argc >= sizeof argv / sizeof *argv) goto done;
    argv[argc++] = token;
  ...
  if (!ehdr_read_success || memcmp(ehdr.e_ident, "\177ELF\2\1\1", 7) ||
      ehdr.e_type != 2 || ehdr.e_machine != 0x3E || ehdr.e_version != 1 ||
  ...
  for (i = 0; i < ehdr.e_phnum; i++) {
    ...
    if (file_ofs < 0 || file_ofs > length) goto done;
    ...
    lock_release(&filesys_lock);
    if (!phdr_read_success) goto done;
...
static bool lazy_load_segment(struct page* page, void* aux) {
  ...
  bytes_read = file_read_at(args->file, kva, args->page_read_bytes,
                            args->ofs);
  ...
  memset(kva + args->page_read_bytes, 0, args->page_zero_bytes);
  free(args);
  return true;
}
