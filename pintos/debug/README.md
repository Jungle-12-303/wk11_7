...
이 폴더는 작은 커널 조각을 직접 검증하는 테스트 파일을 두는 곳이다.
테스트 작성자는 기본적으로 `pintos/debug/**/*.c` 파일만 생각하면 된다.
...
```text
pintos/debug/vm/spt/basic.c
  -> test name: debug-vm-spt-basic
  -> function : debug_test_vm_spt_basic
```
...
- `vm/spt/basic.c`는 `DEBUG_TEST (vm_spt_basic)`를 쓴다.
...
make test TEST=vm/spt/basic
```

출력에 아래처럼 나오면 통과다.

```text
pass debug-vm-spt-basic
...
```sh
make test-debug TEST=basic
```

기본 중단점은 테스트 함수다.

```text
basic -> break debug_test_vm_spt_basic
```

다른 함수에서 먼저 멈추고 싶으면 `BREAK`를 지정한다.

```sh
make test-debug TEST=basic BREAK=spt_find_page
```
...
파일명이 여러 개와 겹치면 `ambiguous debug test`가 나오며, 그때만 `vm/spt/basic`처럼 조금 더 긴 이름을 쓰면 된다.
