...
vm_anon_init (void) {
	...
	/* TODO VM-14: 최소 lazy/stack cycle만 볼 때는 swap_disk 없이 시작할 수
...
anon_swap_in (struct page *page, void *kva) {
	struct anon_page *anon_page = &page->anon;
	/* TODO VM-14: swap slot이 없으면 kva 한 page를 0으로 채운다. slot이
...
anon_swap_out (struct page *page) {
	struct anon_page *anon_page = &page->anon;
	/* TODO VM-21: eviction 전까지는 false/PANIC으로 둘 수 있다. swap 구현
...
anon_destroy (struct page *page) {
	struct anon_page *anon_page = &page->anon;
	/* TODO VM-19: resident frame이 있으면 frame 해제는 공통 destroy 경로와
