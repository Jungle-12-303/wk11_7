...
file_backed_initializer (struct page *page, enum vm_type type, void *kva) {
	...
	/* TODO VM-15: uninit aux 또는 do_mmap() metadata에서 file, offset,
...
file_backed_swap_in (struct page *page, void *kva) {
	...
	/* TODO VM-15: file_page->file의 offset에서 read_bytes만큼 kva로 읽고,
...
file_backed_swap_out (struct page *page) {
	struct file_page *file_page UNUSED = &page->file;
	/* TODO VM-15: writable mmap page가 dirty이면 file_write_at()으로
...
file_backed_destroy (struct page *page) {
	struct file_page *file_page UNUSED = &page->file;
	/* TODO VM-19: process exit/munmap 경로에서 file_backed_swap_out()을
...
do_mmap (void *addr, size_t length, int writable,
	...
	/* TODO VM-15: addr NULL/page-align/user range, length > 0, fd file 유효,
	 * offset page-align, 기존 SPT/PML4/stack/code/data와 겹치지 않음을 먼저
	 * 검사한다. 실패하면 MAP_FAILED를 반환한다. */
	/* TODO VM-15: file_reopen()으로 mapping 전용 file reference를 만들고,
...
void
do_munmap (void *addr) {
	/* TODO VM-16: addr로 시작한 mmap region의 page들을 찾아 dirty page는
